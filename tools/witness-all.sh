...
cd "$(dirname "$0")/.." || exit 1
status=0
for w in typecheck-witness exact-deps-witness meme-coordinates-witness domain-registry-witness surface-parity-witness rite-commands-witness round-trip-witness founding-witness contract-witness; do
  echo "── $w ──"
