...
const BOOT_PROJ = "bags/@lares/ha.ka.ba/@lares/v0.1/api/lares/noosphere-boot.md";
...
describe("ingest quiescence — the composed loop holds still", () => {
  ...
  test("Q4 — the NFC membrane assertion refuses foreign normal forms loudly", async () => {
    if (lar.mode !== "staged") return;
    const nfdPath = join(lar.root, "bags/@lares/ha.ka.ba/@lares/v0.1/api/lares/nfd-probe.md");
    ...
    expect(nonNfc).toContain("lar:///ha.ka.ba/@lares/v0.1/api/lares/nfd-probe");
    expect(d["changed"]).toBe(0);
    expect(d["new"]).toBe(0);
  }, 60_000);
});
