...
After Stafford Beer: **the purpose of a system is what it does.** ~:confidence[C],[20]
...
- *Federated MOVE (Path E in HANDOFF).* When peer A asks peer B to accept canon, what conflict-resolution mechanism resolves disagreement? Today the operator decides locally and the git diff signs the canon; what if peer B's canon view differs?
...
- **Layer-separation failure modes.** When the action-handler fails, the composite-store keeps reading; the cap-layer keeps verifying; sync keeps syncing. The bag-as-policy-boundary commitment echoes Frazee's "right to hosting" lesson — failures contain themselves to the layer that broke. Bluesky's 2024 moderation bug demonstrated the value; the architecture protected users on other PDS hosts because the layer-separation held.
...
This meme begins as a draft in wikis/TestWiki/memes/. It tests the Lares S5.8 canon MOVE (residency ACTION) end-to-end:

- Authored at the unstable URI lar:///ha.ka.ba/docs/lares/the-lares-protocols (no @-scope yet).
- Lives in the room bag, gitignored, until a MOVE places it into canon.
- A residency MOVE via `lares act MOVE --title <uri> --to-bag <canonical-bag>` exercises the full cap-gated path the D-arc landed.

When this meme reaches canon, its URI gains a stable @-scope (likely @lares). The MOVE event itself yields a small piece of evidence for the article's argument: a protocol that names *who can MOVE content into canon* and *how* answers the governance question rather than leaving it silent.
