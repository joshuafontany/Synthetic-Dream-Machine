...
**Storage layer compatibility:** ChromaDB stores documents with metadata. Lares state events are structured JSON, not natural language — they resist semantic embedding (a state tuple `{phase: "orient", stance: ["philosopher", "poet"], register:"~:confidence[S],[13]"}` doesn't embed meaningfully alongside conversation text). Two options: (a) store state events in a separate ChromaDB collection with metadata-only filtering, or (b) keep STATE.jsonl as a separate file alongside MemPalace's ChromaDB. Option (b) preserves the integrity of both data models.
