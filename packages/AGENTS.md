...
`@lararium/mesh` carries contracts and graph law. Keep this package TW5-neutral, browser-neutral, and Node-neutral. Put shared types, parsers, `authority` + `browser-authority`, `lar-uris` resolution, Nexus identity primitives, `FfzClock`, presence slot types, `capability` schemas, `AutomergeDocStore` + `composite-store`, `bag-residency` tiers, `LarEventBus`, `MemeProvider`, `Recipe` + `WikiRecipe`, `LarProjectionRegistry`, `PromotionCeremony`, `ReactionGraph`, `CausalIsland` + `island-protocol`, `KumuDevice`, `social-seed` + `social-tiddlers`, and the shared `cold-boot-ceremony` bones here.
