...
const BOOT = join(REPO_ROOT, "bags/@lares/v0.1/api/lares/noosphere-boot.md");
...
describe("ingest-gate — the §6 triangle decides", () => {
  ...
  test("clean content edit, records unmoved → ingest", () => {
    const edited = source.replace("# Entry - Lararium Cold Boot", "# Entry - Lararium Cold Boot (edited)");
    const d = decideIngest({
  ...
  test("both moved → conflict, surfaced never overwritten", () => {
    const diskEdit = source.replace("# Entry - Lararium Cold Boot", "# Entry (disk hand)");
    const recordsMovedRender = canonical.replace("# Entry - Lararium Cold Boot", "# Entry (record hand)");
    const d = decideIngest({
