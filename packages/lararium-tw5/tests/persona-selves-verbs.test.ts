...
describe("the two names land on @persona and read back", () => {
  test("persona-label writes the PRIVATE pet-name and folds it back through persona-selves", async () => {
    ...
    const out = await r.label({ handleIndex: 1, petname: "ksc-work" }, CTX);
    expect(out).toMatchObject({ verb: "persona-label", handleIndex: 1, petname: "ksc-work", written: true, federated: false });
    expect(await r.selves({}, CTX)).toMatchObject({ selves: [{ handleIndex: 1, petname: "ksc-work" }] });
  });

  test("persona-handle writes the DECLARED Handle beside it, disturbing nothing", async () => {
    const s = store();
    const r = reactorsOver(s);
    await r.label({ handleIndex: 1, petname: "ksc-work" }, CTX);
    await r.handle({ handleIndex: 1, handle: "Telarus, KSC" }, CTX);
    expect(await r.selves({}, CTX)).toMatchObject({
      selves: [{ handleIndex: 1, petname: "ksc-work", handle: "Telarus, KSC" }],
    });
  });
  ...
  test("★ the two names carry SEPARATE stamps — a Handle write never re-stamps the pet-name ★", async () => {
    const s = store();
    const r = reactorsOver(s);
    await r.label({ handleIndex: 0, petname: "the-guru" }, CTX);
    const afterLabel = (await fieldsOf(s, 0))["petname@"];
    await r.handle({ handleIndex: 0, handle: "Guru Joshua Fontany" }, CTX);
  ...
  test("a BLANK value clears one name and leaves the other standing", async () => {
    ...
    await r.handle({ handleIndex: 3, handle: "The Lindwyrm" }, CTX);
    await r.label({ handleIndex: 3, petname: "" }, CTX);
    expect(await r.selves({}, CTX)).toMatchObject({ selves: [{ handleIndex: 3, handle: "The Lindwyrm" }] });
  });
});
...
describe("what the verbs refuse", () => {
  test("★ NEVER FEDERATES — only the persona's own self tiddler is ever written ★", async () => {
    const s = store();
    const r = reactorsOver(s);
    await r.label({ handleIndex: 2, petname: "wyrm-burner" }, CTX);
    await r.handle({ handleIndex: 2, handle: "The Lindwyrm" }, CTX);
  ...
  test("★ the identity machinery beside these labels stays untouched and unread ★", async () => {
    ...
    const r = reactorsOver(s);
    await r.label({ handleIndex: 0, petname: "the-guru" }, CTX);
    expect(await r.selves({}, CTX)).toMatchObject({ selves: [{ handleIndex: 0, petname: "the-guru" }] });
  ...
  test("★ NO verb carries a seat claim — a chair can never be seated from another vessel ★", async () => {
    ...
    const out = await r.handle({ handleIndex: 0, handle: "The Lindwyrm", seat: true }, CTX);
