...
- `◐ M2` — **Read + resolve by declaration.** In `loadCorpora` (`open-node-vessel.ts` + `open-browser-vessel.ts`), read the declared scale off each catalog entry via `parseMeshScale`. For an entry that **declares a mesh scale**, resolve via `resolveBootDoc({tideline:"mesh-shared", scale})` → on `StillJoining`, skip the read-only layer + log (degrade gracefully; background reconcile = M5). **Additive branch — entries with no declared scale keep the current `waitHandleLocal` path, so default boot is unchanged.** **DoD:** a declared-mesh-shared corpus that hasn't arrived skips its layer without minting a blank; test.
