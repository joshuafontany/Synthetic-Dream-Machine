...
def _drawers(source_file: str, turns: list, *, wing: str, room: str,
    ...
    for chunk, ex in enumerate(_assemble_exchanges(turns)):
        ...
        prev_chain = chain
        stratum = ex.get("stratum", "authored")   # non-claude surfaces speak the authored stratum alone
        meta = {
            "wing": wing,
            "room": room,
...
def _claude_stratum(role: str, text: str) -> str:
    """Which stratum a turn speaks from. A `user` turn opening with a harness marker carries machine
    text, never the operator's hand — the channel says `user`, the stratum says otherwise."""
    ...
    with open(path, encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            turns.append({"uuid": uuid, "role": role, "text": text, "ts": ts,
                          "stratum": _claude_stratum(role, text)})
    return turns
