...
> Tests: mesh 254 · tw5 73 · node 94 · browser 20 — all green; typecheck 6/6. Use `pnpm test:fresh` after source changes.
> Next: V3 edge composition (node connector → browser/CLI → gate verify → enforcement flip). Ranked threads: `project_next_vectors`.
...
- **@personal / @draft binding (S7.5+S7.6)** — `resolveOrMintBinding` (create→registerBag→delegate→tiddler); first live `keyhive.delegate`; @personal/@draft wired into the island resolver; `computeRecipeFingerprint` (excludes @lares/@lararium). S7.7 cross-device tests remain. PersonGroup-liveness pre-delegation design space: `bags/@lares/v0.1/api/lararium/personal-slot.md#persongroup-liveness` (pre-delegate caps to an always-on device so the fleet acts while the authoring device sleeps; `project_asymmetric_peer_handoff`).
...
| 2 | **Relay access rings (residency access model)** | ⬜ Epic | The downstream model the isomorphic-vessel work enables. bag = unit of access in 3 rings mapping keyhive membership (Nexus⊃PersonGroup⊃vessel); cascade = unit of view; **anon-read is gate POLICY, not a keyhive grant** (keyhive can't express world-readable). Tiered relay gate / ring-keyed sharePolicy ({@lararium,@lares} read-to-all; escalate auth for write+private); progressive auth not default-deny; published-wiki = residency/visibility action. See [project_relay_access_rings] memory. **Cross-peer task-handoff design space:** `bags/@lares/v0.1/api/pono/federated-causal-islands.md#task-handoff` (asymmetric peers hand a task to a capable peer via signed cap-invocation + CRDT-receipt; see `project_asymmetric_peer_handoff`). |
