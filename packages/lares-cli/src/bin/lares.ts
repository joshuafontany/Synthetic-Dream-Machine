...
const COMMANDS: readonly Command[] = [
  ...
  { name: "edge",          summary: "Set one RELATIONSHIP aside, or take the marker back down \u2014 the k\u0101pae raised over an EDGE rather than over a party. `edge kapae <edge-id> --epoch <e>` shadows that relation; `edge un-kapae <edge-id> --epoch <e>` re-admits it as a deliberate signed act. Scoped under `edge` to mirror `nexus kapae`/`nexus un_kapae` and stay apart from them: that pair shadows a PRESENTER under a kahu quorum, this pair shadows one RELATIONSHIP under whichever key holds it. Setting a relation aside says NOTHING about either end \u2014 the vessel keeps standing, the face keeps standing, only that relation stops counting. A raised marker WINS a same-version tie, so an eviction never quietly reverses when a partition heals. The write asserts no authority: it signs with a named persona root, and whether that root holds the edge gets decided by whichever reader consults the shadow.", handler: cmdEdge },
