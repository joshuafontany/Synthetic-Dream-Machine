...
import { cmdProjectMd }               from "../commands/project-md.js";
...
const COMMANDS: readonly Command[] = [
  ...
  { name: "project-md",    summary: "Render a spec carrier to its submission pair — <name>.md + <name>.md.meta — a standards reviewer reads with no grammar taught. Deterministic: the meta carries the source address + block check and no clock, so a witness proves a pair current by re-projecting. Same mouth as the PROJECT-MD wiki verb. `--out <dir>` redirects; default lands beside the source.", handler: cmdProjectMd    },
  { name: "normalize",     summary: "Canonicalize a meme carrier's framing — the meta namespace into the SOH, the declaration to the one that names the grammar — so the round-trip laws hold. `--check` reports drift without writing (CI/pre-commit).", handler: cmdNormalize     },
