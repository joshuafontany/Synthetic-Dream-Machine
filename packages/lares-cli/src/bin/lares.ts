...
const COMMANDS: readonly Command[] = [
  ...
  { name: "nexus",         summary: "The Nexus founding-kahu ROSTER + its PRE-ROTATED charter-epoch chain (TUF≈KERI) — the Kapae immune antigen's authority home (#68). `nexus charter seat` seats the held personas' ed25519 VERIFYING keys (read from the vault, never the seed; matched by pet-name) + establishes the genesis epoch with a `--next-key-commit` pre-rotation; `nexus charter rotate` reveals the pre-committed next key-set + advances the chain (FAIL-CLOSED on reveal mismatch); `nexus charter commit --keys` computes a commitment digest; `nexus charter show` reads the roster, chain head, + quorum verdict.", handler: cmdNexus },
