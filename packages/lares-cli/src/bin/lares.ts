...
import { cmdKapae }                    from "../commands/kapae.js";
...
const COMMANDS: readonly Command[] = [
  ...
  { name: "cabal",         summary: "The JOIN AXIS — a mutual-hold relation with a cabal-realm, orthogonal to the CARRIAGE contract `nexus contract` writes. `cabal vouch <joiner-nym> --place <realm-doc-id> [--expires <iso>] [--as <root-index>]` stakes ONE held face's OWN standing on one joiner, onto the Nexus vouch board — no kahu quorum, because a vouch is one hand's own stake, not a steward act. It ADMITS NOBODY: a vouch is signal-2 on the lineage the admission price walks, and the cost is paid at the moment of vouching, since a voucher's score SPLITS across everyone they vouch for. Re-vouching the same joiner stays ONE edge — re-minting never buys out-degree.", handler: cmdCabal },
  { name: "kapae",         summary: "Set one RELATIONSHIP aside, or take the marker back down — the k\u0101pae raised over an EDGE rather than over a party. `kapae raise <edge-id> --epoch <e>` shadows that relation; `kapae lower <edge-id> --epoch <e>` re-admits it as a deliberate signed act. Setting a relation aside says NOTHING about either end: the vessel keeps standing, the face keeps standing, and only that relation stops counting. A raised marker WINS a same-version tie, so an eviction never quietly reverses when a partition heals; nothing un-shadows by accident. The write asserts no authority — it signs with a named persona root, and whether that root holds the edge gets decided by whichever reader consults the shadow.", handler: cmdKapae },
