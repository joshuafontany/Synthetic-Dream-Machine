...
const COMMANDS: readonly Command[] = [
  ...
  { name: "capture",       summary: "FEED the telemetry nalu — route a transcript's NEW turns THROUGH the @daemon into the sovereign memory sensorium: each turn → `capture` verb → capture cap → WAL → the caller-vector flush. Idempotent (per-wing watermark). Daemon down → the turns SUSPEND; the transcript is the durable producer-log and the next run re-derives them. `lares capture <transcript|stageDir> --wing <w>`. The drawer leg of the ingest hook.", handler: cmdCapture },
