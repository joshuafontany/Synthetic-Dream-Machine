...
const COMMANDS: readonly Command[] = [
  ...
  { name: "harvest",       summary: "Idempotent session-history harvest (the @admin memory-shore). `--all`: backfill EVERY ~/.claude project — discover, derive per-project wing, mine convos + write lar_* metadata, all idempotent. `--writeback --wing <w>`: enrich one wing's existing drawers with lar_* metadata + hall routing. Default: transcript turns → gradient parser → watermarked bearing index. Re-runnable/resume-safe (lar_hv). `lares harvest [--all | --writeback --wing <w>] [--dry-run]`.", handler: cmdHarvest },
