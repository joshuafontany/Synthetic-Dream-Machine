/**
 * `lares kapae {raise | lower} <edge-id> --epoch <e> [--as <root-index>] [--version <n>]` — set one
...
function usage(): number {
  console.error("usage: lares kapae {raise | lower} <edge-id> --epoch <epoch> [--as <root-index>] [--version <n>]");
  console.error("");
  console.error("  raise <edge-id>   set the relationship aside (a raised marker wins a tie under partition)");
  console.error("  lower <edge-id>   take the marker back down — a deliberate re-admission, never a fall-through");
  return 2;
}

export async function cmdKapae(args: ParsedArgs): Promise<number> {
  const verb = args.positional[0];
  if (verb !== "raise" && verb !== "lower") return usage();
  ...
  try {
    const r = await runEdgeKapae({
      edgeId, epoch, raised: verb === "raise",
