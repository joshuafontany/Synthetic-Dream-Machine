/**
 * persona-scope — the per-handle NAME of a PersonaGroup's private plane.
 *
 * ── THE PROBLEM ─────────────────────────────────────────────────────────────────────────────────
 * `@persona` holds ONE PersonaGroup's private plane: its multitude, signer DID, KEL prefix, device
 * delegation. A human runs SEVERAL PersonaGroups across one set of devices — a work handle and a play
 * handle on the same laptop and the same phone — so one laptop needs one plane per handle. A constant
 * bag id gives it one shelf where the model wants a family, and the second handle has nowhere to stand.
 *
 * ── WHY A MAP WOULD HAVE BEEN THE WRONG ANSWER ──────────────────────────────────────────────────
 * The obvious cure — one vessel-level index that lists a person's handles and points each at its plane —
 * builds the thing canon forbids. The persona-circle ruling: a human's collection of handles is a private
 * VAULT, and NOTHING stands above them that links them, because a layer above the vaults would BE the
 * Nymwars root. SDSI says the same from the naming side (RFC 2693; Stiegler): a petname system is a
 * NAMESPACE, not a super-key — a key placed over the handles becomes a presentable root that correlates
 * them. So the answer must let a vessel FIND each plane without ever holding a thing that ENUMERATES them.
 *
 * ── THE ANSWER, AND THE FOUR BLIND DOMAINS THAT ALREADY AGREE ON IT ─────────────────────────────
 * DERIVE the name from the handle's OWN material, one-way, with nothing above it. Each PersonaGroup's
 ...
 * joins. This module carries that convention up one level — from the KEY a handle presents to a circle, to
 * the NAME a handle's own plane answers to. One convention, two levels, so neither invents a second law.
 *
 ...
 * buys is that the name carries no vessel material: two handles on one laptop produce two bag ids with
 * nothing of the laptop, and nothing of each other, inside either one.
 *
 * A peer may still ask a vessel whether it resolves a given bag, and a vessel that answers two probes has
 * told the asker it holds both handles. That probe surface rides the transport, not the name, and it stays
 * named here rather than quietly assumed away.
 *
 * ── STILL OWED ──────────────────────────────────────────────────────────────────────────────────
 ...
 *  own handful of handles could reach, and short enough that a person reads a bag id in a log. */
 ...
 * `@persona/g/<tag>/…` would put one handle's plane inside another's document, where a cap that opens the
 * outer bag opens every handle a person holds. Distinct bags keep the compartments the vault requires.
 */
...
/** Whether a bag id names some handle's persona plane — the shape a router matches, never a hard-coded id. */
