...
<<~ &#x0001; ? -> lar:///ha.ka.ba/@lararium/mesh/ffz-clock >>
```toml iam
cacheable   = true
...
<<~ aka lar:///ha.ka.ba/@lararium/api/agent-worldline >>
...
- **The actorId slip** — `FfzClock.actorId` keys on the Automerge actor (code header +
  field doc); for a worldline clock it must re-key on the handle (lar:///ha.ka.ba/@lararium/api/agent-worldline#open).
