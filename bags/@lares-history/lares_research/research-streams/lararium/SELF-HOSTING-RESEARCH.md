...
**Lararium adoption:** The `lares/ha.ka.ba/@lares/api/` carriers form the shadow layer. Grammar memes in `lares/grammars/` form the shadow grammar. User-authored carriers can OVERRIDE grammar rules at a different URI but cannot DELETE the shadow grammar. `loadGrammarRules()` reads the shadow grammar; user overrides layer on top. `/admin/reseed` is safe because shadow grammar regenerates from `lares/` on every compile — it is the embedded JSON equivalent.
...
**Lararium adoption from Self:** The prototype chain maps cleanly to Lararium's meme override model. A user meme at `lar:///user:joshu@local/grammars/memetic-wikitext` extends the shadow grammar at `lar:///ha.ka.ba/@lares/api/grammars/memetic-wikitext` — same override pattern as Self's parent slot.
...
|---|---|---|
| Shadow layer (read-only defaults) | TW5 | `lares/ha.ka.ba/@lares/api/` + `lares/grammars/` — regenerable from lares/, never deletable via canvas edit |
...
<<~ pranala #to-grammar ? -> lar:///ha.ka.ba/@lares/api/grammars/memetic-wikitext family:control role:informs >>
