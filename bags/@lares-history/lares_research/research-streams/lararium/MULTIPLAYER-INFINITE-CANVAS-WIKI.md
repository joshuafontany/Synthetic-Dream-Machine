...
**Room WebSocket connections are NOT edge islands.** A room connection is session-scoped and ephemeral. An edge island is a named, capability-gated, durable-offset causal boundary between two Lares nodes — a different architectural layer (see `lar:///ha.ka.ba/@lares/api/pono/federated-causal-islands`).
...
|---|---|---|
| Invariant meme | `lar:///ha.ka.ba/@lares/api/pono/${name}` | `lar:///ha.ka.ba/@lares/api/lararium/the-altar-fire` |
...
| Assign `lar:///ha.ka.ba/@lares/api/...` URI | — | — | — | ✓ |
...
| `tag[uri]` | ✓ shipped | `[tag[lar:///ha.ka.ba/@lares/api/pono/loci]]` |
| `nsort[field]` | ✓ shipped | `[nsort[depth]]` |
| `limit[n]` | ✓ shipped | `[limit[5]]` |
| `prefix[str]` | ✓ shipped | `[prefix[lar:///ha.ka.ba/@lares/api]]` |
...
**Template namespace:** `lar:///ha.ka.ba/@lares/api/lararium/templates/` — inside the stable tuple root, no adjacent namespace. `templates/index` owns control edges to all five carriers and is reachable from the `lararium` meme via `#hydrate-templates`.
...
- ✓ the-altar-fire invariant meme: authored at `packages/lares-core/memes/api/pono/the-altar-fire.md`
...
11. **Orichalcum capability implementation:** Principal identity (Ed25519 device key), Orichalcum capability type + caveat validator, UCAN-compatible wire proof. Server principal stable keypair. Room join gate. See `lar:///ha.ka.ba/@lares/api/pono/orichalcum-capabilities`.
...
**Tier 3 note:** Cross-node federation edges (Tier 3) sit *above* lares/ in the authority hierarchy and *outside* the local Automerge/tldraw room machinery. They are not TLSocketRoom connections. They are named edge islands with their own lifecycle, governed by Orichalcum capabilities. See `#federation` and `lar:///ha.ka.ba/@lares/api/pono/federated-causal-islands`.
...
Full law: `lar:///ha.ka.ba/@lares/api/pono/federated-causal-islands`
Capability model: `lar:///ha.ka.ba/@lares/api/pono/orichalcum-capabilities`
...
<<~ pranala #to-altar-fire ? -> lar:///ha.ka.ba/@lares/api/lararium/the-altar-fire family:control role:defines >>
<<~ pranala #to-canon-promotion ? -> lar:///ha.ka.ba/@lares/api/pono/hooponopono family:control role:governed-by >>
