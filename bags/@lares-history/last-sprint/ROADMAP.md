...
> Updated: 2026-05-12 (S5.9 peer boot parity: Node primary/admin/worker TW5 VMs boot from digest-checked content-addressed core blob)
...
| S5.8 | Promotion Ceremony | ✅ Complete | `@lares/cli` package + `lares act MOVE` CLI; command-tiddler protocol (cmd/=signal, log/=durable record); TS dispatcher + promote handler in `@lararium/node`; CLI ↔ daemon coordination via admin-doc tiddlers (no HTTP/RPC) |
| S5.9 | Peer boot parity | ✅ Complete | Node primary/admin/hot-worker TW5 VMs boot from `LarariumDoc.blobs[ENGINE_CORE_ID]` via `TW5CoreBootBlob` (bytes + sha256 + source), matching browser-peer content-addressed boot intent. `TW5Engine` verifies digest pre-eval, denies fs/wiki-folder ingress, narrows host affordances, and keeps tiddler ingress on preloads/adaptors. Quine gate green. |
...
| H | Save-side auto-split | ✅ Complete (E.10.8 + 2026-05-11) | `<$lar-meme-split>` widget (wiki change event path) + `MemeSyncAdaptor` direct handler (CRDT store path): `splitBodyTiddler()` exported from `deserializer.ts`; `saveTiddler` detects `<<~ ahu` blocks, splits into parent + child records, tombstones removed children. ONE parser, FOUR call sites law complete. |
...
- [x] Delete `api/grammars/` tree entirely
...
- [x] Update `lares/voices.md` mask references to `api/lares/masks`
...
| `lararium-node` | `@lararium/node` | Local host peer: filesystem, operator key, canon MOVE, serve/CLI |
