...
**Lararium adoption:** The `lares/ha.ka.ba/@lares/api/v0.1/` carriers form the shadow layer. Grammar memes in `lares/grammars/` form the shadow grammar. User-authored carriers can OVERRIDE grammar rules at a different URI but cannot DELETE the shadow grammar. `loadGrammarRules()` reads the shadow grammar; user overrides layer on top. `/admin/reseed` is safe because shadow grammar regenerates from `lares/` on every compile — it is the embedded JSON equivalent.
...
|---|---|---|
| Shadow layer (read-only defaults) | TW5 | `lares/ha.ka.ba/@lares/api/v0.1/` + `lares/grammars/` — regenerable from lares/, never deletable via canvas edit |
