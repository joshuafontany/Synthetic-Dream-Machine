...
> Goal: Promote the full registry, resolver, promotion ledger, and URI assignment workflow to `~:confidence[C],[19]`. Settle module/tool/permission schemas. Define the simplified deployment model.
> Status: `~:confidence[S],[13]` — pre-brief; not yet open
> Entry: URI + crystal + invariants at `~:confidence[C],[19]` (Sprints 0–2 complete)
...
- Version/hash scheme: SHA-256 as primary identity; semver as human label via `version_label` field (REG-05, R2 resolved at `~:confidence[CS],[17]`)
- Promotion event recording: append-only (REG-07)

**R3 resolution `~:confidence[CS],[16]`:** Promotion ledger = design tree (`lares/registry/`). Session audit ledger = crystal tree (`lares/STATE.jsonl`). Different concerns, different locations.

### 2. PROMOTION_WALKTHROUGH.md `~:confidence[C],[19]`
...
### 3. MODULE_SCHEMA.md `~:confidence[C],[19]`
...
### 4. TOOL_SCHEMA.md `~:confidence[CS],[17]`
...
|---|---|---|---|---|
| R1 | Ledger format: JSONL vs markdown | `~:confidence[S],[13]` | Lock JSONL (aligns with STATE.jsonl; MemPalace KG compatible) | Lock |
| R2 | Version scheme: hash vs semver | `~:confidence[CS],[17]` | Hash primary (SHA-256); semver as `version_label`. C-report §4. | Confirm + document |
| R3 | Ledger location: design vs crystal tree | `~:confidence[CS],[16]` | Promotion ledger = design tree; audit = crystal tree | Confirm |
| R4 | Resolver: runtime tool vs design reference | `~:confidence[S],[12]` | Lock design reference for alpha | Lock |

---

## Pre-Loaded Context

### From C-deep-research-report.md `~:confidence[CS],[16]`
...
- `semantic_sha256` (normalized canonical form: UTF-8, LF, trim whitespace, sort TOML keys alphabetically, remove comments, then SHA-256) — **mark `~:confidence[SP],[9]` until tested**. Cannot finalize without a prototype normalization run.
...
C-report `canon` field (0.0–10.0 priority scale) vs D-report priority layer table — two representations of same concept, different field names. Reconcile before SCH-01 promotes. Cross-verify each layer against Kernel `~:confidence[C],[20]` — Kernel wins.
...
- REGISTRY.md at `~:confidence[C],[19]` (operator-confirmed)
- PROMOTION_WALKTHROUGH.md at `~:confidence[C],[19]`
- MODULE_SCHEMA.md at `~:confidence[C],[19]`; TiddlyWiki export format verified
- TOOL_SCHEMA.md + PERMISSION_SCHEMA.md at `~:confidence[CS],[17]`+
- DEPLOYMENT_MODEL.md at `~:confidence[CS],[17]`+
...
*Pre-brief status: content migrated from SPRINT_ROADMAP_1_4.md Rev 3 + updated for Consecration (SESSION_CRYSTAL_20260408 Payload 2). Sprint opens when S2 exits at `~:confidence[C],[19]` / `~:confidence[CS],[17]`+.*
