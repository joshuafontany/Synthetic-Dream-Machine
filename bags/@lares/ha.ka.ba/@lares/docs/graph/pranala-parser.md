...
The pranala law (source: `lar:///ha.ka.ba/@lares/api/pono/pranala`) names four surface forms
...
**Authority:** canonical pranala kānāwai (`lar:///ha.ka.ba/@lares/api/pono/pranala`):
...
```
<<~ ahu #edges >>
  <<~ pranala #preload-l-prime ? -> lar:///ha.ka.ba/@lares/api/mu/l-prime family:control role:owns >>
```
...
|---|---|---|
| Full `lar:///` URI | `lar:///ha.ka.ba/@lares/api/mu/l-prime` | use as-is |
| Full URI with fragment | `lar:///ha.ka.ba/@lares/api/mu#entry` | split into `to_uri` and `to_socket` |
| Relative path | `mu/chao` | prepend current carrier's root (`lar:///ha.ka.ba/@lares/api/`) |
