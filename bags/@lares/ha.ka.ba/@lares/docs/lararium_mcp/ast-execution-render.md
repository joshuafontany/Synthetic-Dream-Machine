...
**`papalohe` and `kukali`:** `papalohe` declares the reaction wire at Orient — a named shape in the graph; Verse `suspends`/`kukali` operates at Act — execution waits until the loop advances. The wire and the wait posture belong to different phases. See `lar:///ha.ka.ba/@lares/docs/pono/research-streams/gaps-tensions` Gap 5.
