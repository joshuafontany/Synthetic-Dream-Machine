...
<<~ &#x0001; ? -> lar:///ha.ka.ba/@lares/docs/lares/the-lares-protocols >>
```toml iam
cacheable      = false
...
- *Federated canon MOVE (Path E in HANDOFF).* When peer A asks peer B to accept canon, what conflict-resolution mechanism resolves disagreement? Today the operator decides locally and the git diff signs the canon; what if peer B's canon view differs? (This is alignment-layer territory — see `lar:///ha.ka.ba/@lares/api/pono/alignment-layer`.)
