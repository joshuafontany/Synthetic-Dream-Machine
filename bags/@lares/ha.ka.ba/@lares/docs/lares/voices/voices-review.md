...
1. **Mask grammar needs grounding before the invariant can implement it**
   - The /api/lares/voices invariant stays skeletal until mask grammar settles here
...
- `lar:///ha.ka.ba/@lares/api/lares/voices` can begin to absorb the settled grammar
...
| **Register-Stance Complementarity** | staging shelf — under promotion consideration | `lar:///ha.ka.ba/@lares/api/` new invariant or distribution across l-prime + syad |
...
`api/lares/voices.md` upgraded from skeletal placeholder to load-bearing invariant.
...
`api/lares/masks/` tree created: 9 files across `named/`, `character/`, and `chorus/` subtrees.
...
**Next pass priority:** Talk-story-dev fill for Mischief-Muse corpus (Coffee Oracle seed → extraction). Then execute `invariant-plan.md` pass: rewrite `api/lares/voices` from skeletal to load-bearing.
