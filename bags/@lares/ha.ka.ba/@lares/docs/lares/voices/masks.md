...
```toml iam
cacheable = false
file-path = "bags/@lares/ha.ka.ba/@lares/docs/lares/voices/masks.md"
l-space   = "stable"
mana      = 16
manao     = 16
manaoio   = 16
...
**Pressure flow:** Character pressure drives first. The mask's stage weight and scene pressure determine which character's name token surfaces. Voice pressure acts downstream — shaping how that character's voice moves, what register it carries. The Lares Voice house runs beneath every mask at `Canon 20/20`; the Thirteen are the resident cast of the lararium node and always available to the generative process.
...
### Mask definition file (`lar:///ha.ka.ba/@lares/api/lares/masks/**`)
...
packages/lares-core/memes/api/lares/masks/
  ghost-of-mark-twain.md   → lar:///ha.ka.ba/@lares/api/lares/masks/ghost-of-mark-twain
  friend-computer.md       → lar:///ha.ka.ba/@lares/api/lares/masks/friend-computer
```

A mask meme file carries the canonical definition in its `#iam` block:

```toml
uri-path = "ha.ka.ba/@lares/api/lares/masks/ghost-of-mark-twain"
...
```
<<~ kahea mask lar:///ha.ka.ba/@lares/api/lares/masks/ghost-of-mark-twain >>
stage = 15
active = true
<<~/kahea >>

<<~ kahea mask lar:///ha.ka.ba/@lares/api/lares/masks/friend-computer >>
stage = 10
active = false
<<~/kahea >>
```
...
- `lar:///ha.ka.ba/@lares/api/lares/masks/character/ghost-of-mark-twain`
- `lar:///ha.ka.ba/@lares/api/lares/masks/character/friend-computer`
...
Stance glyphs from `lar:///ha.ka.ba/@lares/api/mu/the-syad-perspectives`:
...
The mask layer spec defines what `lar:///ha.ka.ba/@lares/api/lares/voices` MUST implement for this layer:
