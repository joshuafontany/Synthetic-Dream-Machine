...
# Pattern Integrities — the research shelf


Nine research spirits swept the field across two days. What follows binds as kupono intent — builds implement *these*, never improvisation.
...
- **Reconciled to the coordinator-verb-plane ruling (2026-07):** the `@daemon` coordinator's verbs fan work across the islands-mesh (TS workers + py nalu-gates/servos) — it introduces edges then steps away, exactly per the lane law above. Stream-processing (telemetry, corpus, AI-session-memory) offloads to py islands and runs there directly; pulling that stream *into* the TS coordinator and back out would enact the "sync hops through the coordinator" anti-pattern this shelf names. The lane law reads the same either way: main sees the edge minted, never every message on it.
...
<<~ loulou lar:///ha.ka.ba/@lares/docs/lares/handoff >>
