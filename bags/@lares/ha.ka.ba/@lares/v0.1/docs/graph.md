...
```toml iam
file-path        = "bags/@lares/ha.ka.ba/@lares/v0.1/docs/graph.md"
mana             = 17
manao            = 17
manaoio          = 16
...
3. **ECS plane pattern.** UEFN's Scene Graph separates `entity` (pure container, no data), `component` (attached capability, owns data and logic), and `prefab` (reusable template, stamps instances). Lararium maps exactly: a **meme** is the entity (URI + path, no business logic), the `control:has` bundle is the component list (each component URI is an attached capability, loci among them), and `lifecycle: template` pranala edges are prefabs (declared intent that the compiler stamps into the walk). Pranala families form separate graph planes, each traversed by its own rules. Collapsing them into one walk loses the plane distinctions.
   ...
   **Three semantic layers — keep distinct:**
   ...
   - **`control:has`** (N edges) — Verse class composition: every URI in `class(Parent, Iface1, Iface2, ...)`, parent and interfaces alike (no is-a parent). ECS analogy: the attached component list. `MemeGraph.memesByComponent()` indexes this.
...
| Meme | Role |
|---|---|
| `lar:///ha.ka.ba/@lares/v0.1/docs/graph/loci` | `PranalaEdge`, `Meme`, and `MemesGraph` data model contracts (see note above for loci interface) |
| `lar:///ha.ka.ba/@lares/v0.1/docs/graph/traversal` | three-tier walk rules, DFS cycle detection, Kahn topological sort, declared-unresolved handling |
| `lar:///ha.ka.ba/@lares/v0.1/docs/graph/pranala-parser` | block form, inline form, sugar expansion, `? ->` resolution, field normalization |
| `lar:///ha.ka.ba/@lares/v0.1/docs/graph/artifacts` | content-addressed SHA256 scheme, three artifact classes, compaction rules |
...
| 4 | `packages/lares-core/memes/docs/graph/**` | this carrier cluster (plan → loci) |
