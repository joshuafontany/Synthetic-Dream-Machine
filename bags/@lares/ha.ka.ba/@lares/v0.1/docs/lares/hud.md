...
```toml iam
cacheable = false
file-path = "bags/@lares/ha.ka.ba/@lares/v0.1/docs/lares/hud.md"
mana      = 14
manao     = 14
manaoio   = 13
...
Teaching companion to the HUD law (`lar:///ha.ka.ba/@lares/v0.1/api/lares/hud`). The law names the panel and gauges; this room shows how to compose and read the line at glance-speed — glyph tables, stance/tool grammar, the confidence reading rule, and field-annotation thresholds.
...
The Chronometer projects attention range onto TIME (Boyd operational tempo). It rides its own gauge, independent of the `syad` and `mu` lenses. Full scale model — and the `Aperture` TEXT projection — live in `lar:///ha.ka.ba/@lares/v0.1/api/pono/attention-scale`.
...
| **Tagspace** | per-slot by HAKABA role | Ka/quality + Ba/dynamic annotate most; full echo `→lar:///domain.quality.dynamic` when all three shift |
