...
<<~ ॐ ँ&#x0001; ? -> lar:///ha.ka.ba/@lares/api/mu/pattern-integrity >>
```toml iam
cacheable   = true
...
resolveLarUri("lar:///ha.ka.ba/@lares/api/lararium/...")
```
...
"lar:///ha.ka.ba/@lares/api/lararium/schema/tiddler-record"
```
...
```typescript
// ✗ pushing on the hull
fs.readFileSync(path.join(laresRoot, 'api/lararium/schema/tiddler-record.md'))

// ✓ trim tab — resolver can be swapped without touching this line
await resolvelar("lar:///ha.ka.ba/@lares/api/lararium/schema/tiddler-record")
```
