...
| term | meaning |
|---|---|
| **carrier** | one `text/x-memetic-wikitext` document. |
...
| **camera** | one render surface over the shared world graph (page or canvas). |

<<~/ahu >>

<<~ ahu #lexical-structure >>
...
| Prefix | Mode | Example |
|---|---|---|
| `<<~` | primary sigil | `<<~ ahu #id >>`, `<<~ aka lar:///uri >>` |
| `<<~/` | block close | `<<~/ahu >>` |
...
open-unres  = "<<~?"
close-mark  = ">>"
block-close = "<<~/" sigil-name LWSP close-mark

; ── attributes ──────────────────────────────────────────────
attr        = key ":" value / URI-ref / fragment / name-token
key         = name-token
value       = DQUOTE *( %x20-21 / %x23-7E ) DQUOTE / name-token
attrs       = *( LWSP attr )

; ── sigils ──────────────────────────────────────────────────
inline      = open LWSP sigil-name attrs LWSP close-mark
block       = open LWSP sigil-name [ LWSP fragment ] attrs LWSP close-mark
              content
              block-close
pragma      = open-pragma LWSP sigil-name [ LWSP params ]
              [ LWSP "=" LWSP filter-expr ] LWSP close-mark
              [ content open-pragma-close ]
unresolved  = open-unres LWSP [ fragment ] LWSP close-mark

; ── edges (sigil-name is an edge-family sugar) ──────────────
edge        = open LWSP sigil-name LWSP [ fragment LWSP ]
              [ "?" LWSP "->" LWSP ] URI-ref attrs LWSP close-mark

; ── guest grammar (opaque to the host) ─────────────────────
filter-expr = "[" *( %x20-5C / %x5E-7E ) "]"   ; evaluated by a guest handler

; ── document ────────────────────────────────────────────────
carrier     = [ header ] *( block / inline / pragma / unresolved / text )
              [ footer ]
header      = open LWSP %x01 LWSP "?" LWSP "->" LWSP URI-ref LWSP close-mark
footer      = open LWSP %x04 LWSP "->" LWSP "?" LWSP close-mark
...
A pranala edge MUST declare a **family** from the canonical set. Families carry role vocabularies and contract flags; the canonical taxonomy lives at `lar:///ha.ka.ba/@lares/api/pono/pranala-families`. The families align to the two render surfaces:
...
A **conforming parser** MUST: accept the surface of #grammar; map aliases to canonicals; produce the graph-layer AST and typed edges; enforce close-tag matching; treat `filter-expr` as opaque.
...
```
Type name:                 text
Subtype name:              x-memetic-wikitext
Required parameters:       none
Optional parameters:       charset (default UTF-8)
Encoding considerations:   8-bit; UTF-8 REQUIRED
Fragment identifier:       a "#name" anchor names a worksite (ahu) within the
                           carrier; resolution defers to [LAR-URI]
...
Applications:              lararium nodes; TiddlyWiki5 carriers; mesh tooling
File extension:            .md (carrier form), .tid (runtime widget/procedure)
```

The `x-` prefix marks the subtype as unregistered with IANA; a future tree-transfer drops the prefix on registration.
