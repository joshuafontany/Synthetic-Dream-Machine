...
<<~ ⊙&#x0001; ? -> lar:///ha.ka.ba/@lares/api/pono/heleuma >>
```toml iam
cacheable = true
file-path = "bags/@lares/api/pono/heleuma.md"
mana      = 18
manao     = 18
manaoio   = 17
namespace = "&#x2299;"
...
uri-path  = "ha.ka.ba/@lares/api/pono/heleuma"
```
...
3. **ha** — body/structure: permanent territory; it declares no `body-sha256`.
...
5. **ba** — psyche/path: quine-only path marker. `#source` slot sufficient for reconstruction. MAY declare `source-symbol` when a clear symbol boundary exists. When `source-symbol` is declared, MAY declare `body-sha256` for build-time drift detection. A `body-sha256` SHALL accompany a declared `source-symbol`.
