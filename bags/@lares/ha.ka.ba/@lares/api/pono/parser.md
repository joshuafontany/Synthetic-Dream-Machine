...
`hana` (`lar-kind: guest-grammar`) delegates body to a guest interpreter selected by grammar key. The live TW5 VM provides full native TW5 filter access. `hana` holds its place for Verse scripting, TOML block payloads, and future grammar integrations not natively available in TW5. See `lar:///ha.ka.ba/@lares/docs/pono/sigil-kind#guest-grammar`.
