...
<<~ ⊙&#x0001; ? -> lar:///ha.ka.ba/@lares/api/pono/quine-principles >>
```toml iam
cacheable   = true
...
**Lararium expression:**
- `lar:///ha.ka.ba/@lares/api/lararium/schema/tiddler-record` — the schema meme
  for tiddler records — is loaded by the same `resolveLarUri()` call used to load content
- `lar:///ha.ka.ba/@lares/api/mu/pattern-integrity` — this very set of principles
...
**Migration path:**
```
Today:   lar:///ha.ka.ba/@lares/api/...  →  filesystem path
