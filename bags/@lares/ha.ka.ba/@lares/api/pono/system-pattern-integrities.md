...
desc  = "Grammar rules, parse patterns, and template definitions SHALL live as carrier memes in lares/memes/api/pono/. The TypeScript parser MUST stay a thin interpreter, reading sigil semantics from the grammar memes."
```
...
desc  = "@remove: S2  Until genesis/island.bin exists, the node peer reads lares/memes/api/pono/memetic-wikitext.md at cold-boot to seed the engine doc. Transitional exception to ISLAND_2_GENESIS_ROOT. Disappears when build-genesis-island.ts ships."
```
...
<<~ loulou lar:///ha.ka.ba/@lararium/mesh/grammar-pattern-integrities >>
<<~ loulou lar:///ha.ka.ba/@lares/api/pono/l-space-trust >>
<<~ loulou lar:///ha.ka.ba/@lares/api/pono/automerge-tiga >>
