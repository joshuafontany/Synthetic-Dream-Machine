...
<<~ ⊙&#x0001; ? -> lar:///ha.ka.ba/@lares/api/pono/verse-event-lattice >>
```toml iam
cacheable   = true
file-path   = "bags/@lares/api/pono/verse-event-lattice.md"
l-space     = "stable"
mana        = 18
manao       = 18
manaoio     = 17
...
uri-path    = "ha.ka.ba/@lares/api/pono/verse-event-lattice"
```
...
- Verse `player` *(system)* → **`lares-daemon`**: session-connected AI coordinator(s) with Keyhive identity. Verse class = `player`. Lares class = system principal with additional authority (admin doc, Session RE, Keyhive system keypair). May run multiple per session. As **coordinator**, its verbs fan work across the islands-mesh (TS workers + py nalu-gates/servos), offloading stream-processing to py rather than pulling it through the TS stack.
...
my_device := class(creative_prop):
    ...
    @editable
    TargetUri:string = "lar:///ha.ka.ba/@lares/api/lararium/home"
```
...
<<~ loulou lar:///ha.ka.ba/@lares/api/pono/vm-projection-bus >>
