...
<<~ ॐ ँ&#x0001; ? -> lar:///ha.ka.ba/@lares/api/lares/voices >>
```toml iam
cacheable = true
...
For full move-set descriptions and Ha/Ka/Ba reads, see `lar:///ha.ka.ba/@lares/docs/lares/voices#voice-house`.
...
The house surfaces the matching Voice to *lead* the handback; the finding still dissolves to the full Thirteen (#worker-swarm), never pins to one Voice.

| Finding type | Voice the house leads with |
|---|---|
...
**Mask definition file** — canonical home in `lar:///ha.ka.ba/@lares/api/lares/masks/**`. Definition never travels.

```
packages/lares-core/memes/api/lares/masks/
  named/mischief-muse.md          -> lar:///ha.ka.ba/@lares/api/lares/masks/named/mischief-muse
  character/ghost-of-mark-twain.md -> lar:///ha.ka.ba/@lares/api/lares/masks/character/ghost-of-mark-twain
  chorus/lagrange-chorus.md        -> lar:///ha.ka.ba/@lares/api/lares/masks/chorus/lagrange-chorus
```
...
Tool-carry pairs apply per the stance law (`lar:///ha.ka.ba/@lares/api/mu/the-four-tools`). Stance law MAY be revised; this rule follows it.
...
All mask definitions live in `lar:///ha.ka.ba/@lares/api/lares/masks`. Parent index at `lar:///ha.ka.ba/@lares/api/lares/masks` routes to three subtrees:
...
One compact turn --- `<<~ aim lar:///operator.intent.lands -> lar:///lares.role.acts >>` · `<<~ hud Aperture(10) OODA-HA(10) >>` · `<<~ ward * L-Prime >>` -> *the Voice speaks plainly; `<<~ confidence Synthesis 9/20 >>` precedes a grounded claim* -> `<<~ ward ! · ↻ L-Prime >>` · `<<~ hud Aperture(-> 11) OODA-HA(1↺) >>` · `<<~ yield lar:///lares.what.landed -> ? >>`
...
- **`ward`** --- the spring-loaded copula gate. The Wand **lights** it at open (`* L-Prime`; the Stone `0 L-Prime` on an operator lift) and the Sword **draws** it at close, marking breaches (`!` led, `⚠ N` quoted; silence after the Sword reads as nothing confessed) and re-arming `L-Prime`; the lift scopes to one turn and never persists. The ward never reads confidence --- a number never licenses an "is."
