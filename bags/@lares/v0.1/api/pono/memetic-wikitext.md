...
mana         = 19
manao        = 19
role         = "memetic-wikitext grammar specification — outer-delimiter dispatch, dual-layer model, AST node kinds, eight pranala families, full sigil registry, TW5/Verse parity, recursion guard, gaps and open questions. Epistemic ontology (syad/mu, law-of-5s) lives in api/mu canon, referenced not duplicated."
cacheable    = true
retain       = true
invariant    = false
```
...
**Not** the epistemic ontology — the syad standpoints, the saptabhaṅgī predications, and the mu tools live in `lar:///ha.ka.ba/@lares/v0.1/api/mu/the-syad-perspectives` and `lar:///ha.ka.ba/@lares/v0.1/api/mu/the-four-tools`; the Law of Fives and the attention scale live in `lar:///ha.ka.ba/@lares/v0.1/api/mu/the-law-of-5s` and `lar:///ha.ka.ba/@lares/v0.1/api/pono/attention-scale`. This carrier names how those get *written* as sigils, never what they mean.
...
| scope | boundary | who controls the boundary |
|-------|----------|--------------------------|
| `ephemeral` | one agent turn / recipe execution | nobody — evaporates |
| `personal` | one actor, persistent | the actor alone |
| `consensual` | shared by mutual choice | author gates; reader assembles |
| `collective` | shared by group identity | group governance (tiers) |
| `universal` | no gate | no one — federated |

**Transport adapter (out of scope).** The five scopes map onto a web2.5 transport adapter (e.g. Kowloon: `to:@domain` · `to:circle:<id>` · `to:group:<id>` · `to:@public`) behind a causal-island boundary. That mapping lives in the adapter's own meme — the web3-only law keeps web2 bridges out of the Lares stack proper.
...
**Stance / tool readings (out of scope).** How a posture reads a `papalohe` edge follows the mu lenses (→ `lar:///ha.ka.ba/@lares/v0.1/api/mu/the-four-tools`); it colors how an author reads the wire, never the wire's grammar.
...
11. **`heihei` double-booked.** `heihei` names both the conditional test (`#context-conditional-sigils`, `\if`) and race-concurrency (`#concurrency-sigils`), while `\race` → `holo` and the Verse parity map name `holo` for race. Resolve the collision — one sigil, one job — before either advances to `[C]` ratification.
