...
| **enableable** | **Hot / Warm / Cold Tier** | Devices implement `Enable()` / `Disable()`. The island-owned thermal axis is `hot → warm → cold` with an orthogonal **pin-flag** (pinned = exempt from cooling; admin / active wiki / Session Wiki are pinned-hot). `Enable()` = `hoʻomahana` (warm-up); `Disable()` = `hoʻoanu` (cool — to **warm** = suspend-not-terminate, resume by signal; or to **cold** = full teardown, resume by spawn + `ea`). Subscription teardown (via `KumuCancelable.cancel()`) MUST precede a cool to cold. Bag residency derives from the warmest referencing island — see `lar:///ha.ka.ba/@lares/v0.1/api/lararium/residency-tiers`. |
...
3. **Main → Worker:** tier signal — `{ schema_version: 1, type: "teardown" | "hoʻomahana" | "hoʻoanu" }`
   - `hoʻomahana` ("to warm") — promote a cool slot toward hot/active tier
   - `hoʻoanu` ("to cool down") — demote a hot slot toward cool/archived tier
   - Warm/Cool duality: pinned tier reads `hot` (operator keeps Pele's mana alive); active reads `mahana` (warm); archived reads `anu` (cool). The transition verbs use the causative `hoʻo-` prefix.
