...
- `E-Prime~N`, `No-Have~N`, `~:p[N]`, and `~:confidence[C],[N]` remain scalar humility instruments, not decorative badges.
  ...
  - `No-Have~N` (1–20): how strongly the span avoids possession collapse (`has` as bounded ownership). Tracked separately because possession drift accumulates beneath E-Prime revision pressure. Preferred form: scoped carrying. Baseline defaults to `1` (minimal) — `no-have` may over-affect design language when composability and the `#has` relation serve critical ontological functions.
