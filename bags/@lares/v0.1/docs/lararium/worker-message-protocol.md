...
<<~ &#x0001; ? -> lar:///ha.ka.ba/@lares/v0.1/docs/lararium/worker-message-protocol >>
```toml iam
uri-path = "ha.ka.ba/@lares/v0.1/docs/lararium/worker-message-protocol"
file-path = "bags/@lares/v0.1/docs/lararium/worker-message-protocol.md"
...
3. **Main → Worker** — tier signal: `{ schema_version: 1, type: "teardown" | "hoʻowela" | "hoʻoanu" }`
   ...
   - The thermal axis runs **two-state** (`wela` / `anu`) with an orthogonal pin-flag (pinned = exempt from cooling); the transition verbs take the causative `hoʻo-` prefix. No warm middle tier exists — see `lar:///ha.ka.ba/@lares/v0.1/api/lararium/residency-tiers#third-tier-gate`.
...
<<~ pranala #cites-residency-tiers ? -> lar:///ha.ka.ba/@lares/v0.1/api/lararium/residency-tiers family:observe role:cites >>
