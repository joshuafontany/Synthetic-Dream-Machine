...
**Not scope:** Invariant law. See `lar:///ha.ka.ba/@lares/v0.1/api/pono/memetic-wikitext` for law,
and `lar:///ha.ka.ba/@lares/v0.1/api/grammars/memetic-wikitext-spec` for the full holistic spec.
...
- TW5 widget/transclusion model: TiddlyWiki5 documentation (tiddlywiki.com). Underlying transclusion and widget grammar serves as ~19 confidence core model for parity mapping. TW5 `$list` widget supplies the nearest analog for `huli`: it displays tiddlers matching a filter, renders each result through body content or a template, and supports binding the current listed title to a variable. Counter variable support exists in `$list` but falls outside the current `huli` approval scope.
...
<<~ pranala #to-grammar-spec ? -> lar:///ha.ka.ba/@lares/v0.1/api/grammars/memetic-wikitext-spec family:control role:extends >>
