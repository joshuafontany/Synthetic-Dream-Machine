...
The sigil layer rides as the RUBRIC beside the propositional stream - the missal's division, *rubrica*, red ochre (the determinative's procedure). **The black speaks; the red acts** - in a stream the red steers, in a wiki it calls: renders, transcludes, invokes. Both DO; only the black SAYS.
...
**Two firings, one axis, different evidence.** The open reads the **pull toward exposure** off upstream context, before content exists, and steers generation against it. The close holds the **artifact** and hands the operator ONE **collidable clue**. A close reporting on its own open has **anchored** — a prediction returned where evidence stood. The two answer to each other not at all.
...
**One test decides the clue: can the operator go collide it?** *"This rests on a source I never opened"* invites a collision. *"I may have been biased"* invites nothing.
