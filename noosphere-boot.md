...
The number vows alone; the band-vocabulary behind the `register` iam field lives at the framing spec (`lar:///ha.ka.ba/lares/api/pono/memetic-wikitext-framing`). The scale reads as regions, not bins - a `<<~ confidence 12/20 >>` vow sits near the firming edge, ready to firm or to soften. The highest Confidence stays reserved for talk-story consensus.
